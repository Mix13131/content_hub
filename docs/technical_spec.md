...
В текущей реализации публичный API сайта read-only и не требует admin token. Публичными считаются посты со статусами `queued`, `partially_published`, `published`; посты со статусом `error` не отдаются. Медиа в публичном API metadata-only: Telegram `file_id`, `file_unique_id` и `storage_key` не возвращаются, `file_url` пока `null`.
